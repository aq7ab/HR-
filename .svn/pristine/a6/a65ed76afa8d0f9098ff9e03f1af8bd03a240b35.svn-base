...
namespace DAO
{
    public class UserDAO
    {
        ...
        public  int Rs()
        {
            using (SqlConnection ss = new SqlConnection(conStr))
            {
                string sql = $@"select count(u_id) from [dbo].[users]";

                return ss.QueryFirst<int>(sql);
            }
        }
        public int Rs1()
        {
            using (SqlConnection ss = new SqlConnection(conStr))
            {
                string sql = $@"select count(huf_id) from [dbo].[human_file]";
        ...
        public int Rs3()
        {
            using (SqlConnection ss = new SqlConnection(conStr))
            {
                string sql = $@"select count(ssd_id) from[dbo].[salary_standard]";

                return ss.QueryFirst<int>(sql);
            }
        }

        public async Task<List<SG>> Rs4()
        {
            using (SqlConnection connection = new SqlConnection(conStr))
            {
                string sql = $"SELECT convert(nvarchar(10),regist_time,120) regist_time,COUNT(1) AS Number,sum(salary_paid_sum) salary_paid_sum\r\nFROM [dbo].[salary_grant]\r\nWHERE regist_time BETWEEN '2000-10-02' AND '2025-10-07'\r\nGROUP BY convert(nvarchar(10),regist_time,120)\r\nORDER BY regist_time";
                return (List<SG>)await connection.QueryAsync<SG>(sql);
            }
        }

        public List<Fill> Rs5()
        {
            using (SqlConnection connection = new SqlConnection(conStr))
            {
                string sql = $"select top 7 * from [dbo].[human_file]  order  by  [paid_salary_sum] desc";
                return (List<Fill>)connection.Query<Fill>(sql);
            }
        }
        ...
        public int Rs2()
        {
            using (SqlConnection ss = new SqlConnection(conStr))
            {
                string sql = $@"select count(ffk_id) from [dbo].[config_file_first_kind]";
                int yi = ss.QueryFirst<int>(sql);
                string sql1 = $@"select count(fsk_id) from [dbo].[config_file_second_kind]";
                int yi1 = ss.QueryFirst<int>(sql1);
                string sql2 = $@"select count(ftk_id)from [dbo].[config_file_third_kind]";
                int yi2 = ss.QueryFirst<int>(sql2);
                int yy = yi + yi1 + yi2;
                return yy;
            }
        }
